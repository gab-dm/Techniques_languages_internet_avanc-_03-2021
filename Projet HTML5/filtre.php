...
$trouver='';
$cpt_info=3;// indique sur quelle colonne se trouve l'info qui nous intéresse
...
if($_GET['mot_clef'] != 'none'){
    ...
    if ($trouver == ''){
    echo 'mot clef non valide, mot clef par default selectionné '; // si le mot clef ne correspond à rien, tout est affiché et ce message s'affiche 
    }
}
...
if($_GET['caracteristiques'] == 'Def'){// si l'utilisateur ne selectionne pas de caracteristique
    if($trouver!=''){ // si l'utilisateur a mis un mot clef valide
        if($_GET['Meridien'] != 'Def'){ // si l'utilisateur selectionne un Meridien
        ...
        else{ // si l'utilisateur ne selectionne pas de Meridien
    ...
    else{ // si l'utilisateur n'utilise pas de mot clef
        if($_GET['Meridien'] != 'Def'){// si l'utilisateur selectionne un Meridien
...
else{ // si l'utilisateur  selectionne des caracteristiques
    if($trouver!=''){
...
$reponse = $dbh->prepare($sql);// prépare la requète
$reponse->execute($filtre); // exécute la requète
...
// envoie le tableau à la page tpl
